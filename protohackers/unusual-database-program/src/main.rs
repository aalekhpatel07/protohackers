    ...
    fn from(v: String) -> Self {
        match v.find('=') {
            Some(index) => {
                ...
                Operation::Insert { key: key.to_string(), value: value.chars().skip(1).collect() }
            },
            None => {
                Operation::Retrieve { key: v }
            }
        }
    }
}
        ...
        if let Some((operation, response_tx)) = operations_rx.recv().await {
            ...
            let response = match operation {
                Operation::Insert { key, value } => {
                    guard.insert(key.as_str(), value.as_str())
                },
            ...
            tokio::task::spawn(async move {
                match rx.await {
                    Ok(Some(maybe_response)) => {
                        ...
                        socket_send.send_to(maybe_response.as_bytes(), remote_addr).await.unwrap();
                    },
                    Ok(None) => {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    _ = tokio::join!(
        operations_handle,
        run_server(operations_tx, 12002)
    );
